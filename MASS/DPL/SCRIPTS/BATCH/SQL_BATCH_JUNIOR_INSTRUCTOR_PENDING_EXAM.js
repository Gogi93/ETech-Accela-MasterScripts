/***********************************************************************************************************************************
* @Title 		: 	SQL_BATCH_JUNIOR_INSTRUCTOR_PENDING_EXAM
* @Author		:	Sagar Cheke
* @Date			:	03/29/2015
* @Description 	:	If the workflow has been sitting in the status 'Approved to sit for Exam' for more than 2 years Then close the application
***********************************************************************************************************************************/
...
var stagingConfigurationString = '{\
	"connectionSC": "DB_CONNECTION_INFO",\
		"supplemental":   [{\
					"tag":"queryINPendingExamApps",\
					"procedure":{\
						"name":"ELP_SP_HD_IN_PENDINGEXAM_QUERY",\
						"resultSet":{"list":[\
	...
	if(dbConfiguration)
	{
		...
		*/
		...
		for (var ii = 0; ii < stagingConfiguration.supplemental.length; ii++ )
		{
			...
			if (supplementalConfiguration.tag == "queryINPendingExamApps")
			{
		...
		{
			var message = "Cannot find procedure queryINPendingExamApps";
			var exception = new Error(message);
			throw exception;
		}
		aa.print("Found queryINPendingExamApps: " + supplementalConfiguration.procedure.name);
		...
		*/
		...
		//for (var queryResult = dataSet.next(); queryResult != null; queryResult = dataSet.next()){
			...
			if((daysDiff >= 730) || (daysDiff >= 731) )
			{
				aa.print("Update Workflow status of the application to Expired and close the application");
				
				updateTask("Exam", "Expired", "Updated via batch script.", "Updated via batch script.");
				closeTask("Exam", "Expired", "Updated via batch script.", "Updated via batch script.");
...
catch (ex)
{
   ...
   aa.env.setValue("EMSEReturnMessage", "Error executing SQL_BATCH_JUNIOR_INSTRUCTOR_PENDING_EXAM" + ex.message);
   aa.print("EMSEReturnCode: " + ScriptReturnCodes.OTHER);
   aa.print("EMSEReturnMessage: " + "Error executing SQL_BATCH_JUNIOR_INSTRUCTOR_PENDING_EXAM. " + ex.message);
...
finally 
{
	...
	if (!ELPLogging.isFatal()) 
	{
		...
		if (ELPLogging.getErrorCount() > 0) {
			aa.env.setValue("EMSEReturnMessage", "SQL_BATCH_JUNIOR_INSTRUCTOR_PENDING_EXAM completed with " + ELPLogging.getErrorCount() + " errors.");
			logDebugAndEmail("EMSEReturnMessage: " + "SQL_BATCH_JUNIOR_INSTRUCTOR_PENDING_EXAM completed with " + ELPLogging.getErrorCount() + " errors.");
		} else {
			aa.env.setValue("EMSEReturnMessage", "SQL_BATCH_JUNIOR_INSTRUCTOR_PENDING_EXAM completed with no errors.");
			logDebugAndEmail("EMSEReturnMessage: " + "SQL_BATCH_JUNIOR_INSTRUCTOR_PENDING_EXAM completed with no errors.");					
		}
	}
 ...
 */
function dateDifferenceForBatch(date1, date2) 
{
...
function getRecordsArray(emseParameters){
	var sql = 
			...
			WHERE  b.b1_per_group = 'License' \
			       ...
			       AND b.b1_per_sub_type = 'Instructor' \

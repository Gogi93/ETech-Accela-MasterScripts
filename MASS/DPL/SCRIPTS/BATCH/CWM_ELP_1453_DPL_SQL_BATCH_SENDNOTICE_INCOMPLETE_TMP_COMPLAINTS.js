/* ------------------------------------------------------------------------------------------------------ /
 | Program : CWM_ELP_1453_DPL_SQL_Batch_SendNotice_Incomplete_TMP_Complaints Trigger : Batch
 |
 ...
 | - None
 | Batch Options:
 | - NO PARAMS - Default daysToPurge = 45
...
var daysToPurge = lookup("BATCH_PURGE_TEMP", "COMPLAINTTEMP");
...
try
{
	...
	*/
	...
	//for (var queryResult = dataSet.next(); queryResult != null; queryResult = dataSet.next())
	{
		...
		ObjKeyRename(dataSet[i], {"TMP_COMPLAINT": "tmpComplaint"});
		...
		aa.print(queryResult.daysSinceLastUpdate + " " + queryResult.id1 + "-" + queryResult.id2 + "-" + queryResult.id3 +  " (" + queryResult.tmpComplaint + ")" + " created date:" + queryResult.recDate);

		capIdResult = aa.cap.getCapID(queryResult.tmpComplaint);
		...
		if( queryResult.daysPastCreatedDate >= daysToPurge )
		{
			ELPLogging.debug(altId + " is " + queryResult.daysPastCreatedDate + " day past created date");


			logDebug("Looping through CAP contacts.");
			sendComplainantAbandonNotice(capId)
			//ELPLogging.debug("Successfully sent TMP complaint " + queryResult.tmpComplaint);			
			deletedTMPCapCount++;
		}				
				
	} // end for loop over the Oracle Data set returned.

	logDebugAndEmail("Total TMP Complaints past half way purge date: " + capCount);
	logDebugAndEmail("Emailed # TMP Renewals: " + deletedTMPCapCount);	
...
catch (ex)
{
   ...
   aa.env.setValue("EMSEReturnMessage", "Error executing CWM_ELP_1453_DPL_SQL_Batch_SendNotice_Incomplete_TMP_Complaints" + ex.message);
   ELPLogging.debug("     EMSEReturnCode: " + ScriptReturnCodes.OTHER);
   ELPLogging.debug("     EMSEReturnMessage: " + "Error executing CWM_ELP_1453_DPL_SQL_Batch_SendNotice_Incomplete_TMP_Complaints. " + ex.message);
   
   var returnException = new ELPAccelaEMSEException("Error executing CWM_ELP_1453_DPL_SQL_Batch_SendNotice_Incomplete_TMP_Complaints " + ex.message, ScriptReturnCodes.EMSE_PROCEDURE);
   ELPLogging.fatal(returnException.toString());
   throw returnException;
}
finally 
{
	...
	if (!ELPLogging.isFatal()) {
		...
		if (ELPLogging.getErrorCount() > 0) {
			aa.env.setValue("EMSEReturnMessage", "CWM_ELP_1453_DPL_SQL_Batch_SendNotice_Incomplete_TMP_Complaints completed with " + ELPLogging.getErrorCount() + " errors.");
			logDebugAndEmail("EMSEReturnMessage: " + "CWM_ELP_1453_DPL_SQL_Batch_SendNotice_Incomplete_TMP_Complaints completed with " + ELPLogging.getErrorCount() + " errors.");
		} else {
			aa.env.setValue("EMSEReturnMessage", "CWM_ELP_1453_DPL_SQL_Batch_SendNotice_Incomplete_TMP_Complaints completed with no errors.");
			logDebugAndEmail("EMSEReturnMessage: " + "CWM_ELP_1453_DPL_SQL_Batch_SendNotice_Incomplete_TMP_Complaints completed with no errors.");					
		}
	}
...
//takes in tempCapId and sends out notice to Complainant type that the complaint record is incomplete and will be abandoned
function sendComplainantAbandonNotice(pCapId)
{
	var contactArr=aa.people.getCapContactByCapID(capId).getOutput();
	for(var xx in contactArr)
	{
		aa.print(xx+"  "+contactArr[xx]);

		var xCapContactModel = contactArr[xx].getCapContactModel();
		aa.print(xCapContactModel);
		if(xCapContactModel.getContactType()=="Complainant")
		{
			aa.print(xCapContactModel.getContactType());
			if( xCapContactModel.getEmail()!=null)
			{
				aa.print(xCapContactModel.getEmail());
				//dumpObj(xCapContactModel);
				...
				fvEmailParameters.put("$$First Name$$",xCapContactModel.getFirstName());
				fvEmailParameters.put("$$Last Name$$",xCapContactModel.getLastName());
				fvEmailParameters.put("$$ACAUrl$$",vACAUrl);
		
				if(!aa.document.sendEmailAndSaveAsDocument(sysFromEmail,xCapContactModel.getEmail(),"","ACA COMPLAINT INCOMPLETE NOTICE",fvEmailParameters,fvCapID4Email,fvFileNames))
				 aa.print("Error Sending Notice");
			}
		}
	}
}
...
function getRecordsArray(emseParameters){
	var sql = 
			"SELECT SERV_PROV_CODE, B1_PER_ID1, B1_ALT_ID, B1_PER_ID2, B1_PER_ID3, B1_PER_GROUP, B1_PER_TYPE, B1_PER_SUB_TYPE, \
			B1_PER_CATEGORY, REC_DATE, B1_ALT_ID as TMP_COMPLAINT, FLOOR(TRUNC(SYSDATE) - REC_DATE) AS DAYS_SINCE_LAST_UPDATE \
			FROM b1permit \

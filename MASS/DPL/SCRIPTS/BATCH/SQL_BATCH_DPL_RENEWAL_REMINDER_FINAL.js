/* ------------------------------------------------------------------------------------------------------ /
 | Program : SQL_BATCH_DPL_RENEWAL_REMINDER Trigger : Batch
 |
...
var renewalSetName = "DPL|RENEWAL_REMINDER";
 ...
 * */
var emailAddress = lookup("BATCH_STATUS_EMAIL", "RENEWAL REMINDER"); // This email will be set by standard choice
...
var stagingConfigurationString = '{\
	"connectionSC": "DB_CONNECTION_INFO",\
		"supplemental":   [{\
					...
					"procedure":{\
						"name":"ELP_SP_RENEWAL_REMINDER_QUERY",\
						"resultSet":{"list":[\
...
try
{
	...
	*/
	...
	for (var i in dataSet) {
		...
		ObjKeyRename(dataSet[i], {"START_RANGE":"start_range"});
		ObjKeyRename(dataSet[i], {"END_RANGE":"end_range"});
	...
	//for (var queryResult = dataSet.next(); queryResult != null; queryResult = dataSet.next()){
		if(firstRec)
		{
			logDebugAndEmail("Searching for records with expiration date range from: " + queryResult.start_range + " to: " + queryResult.end_range );
		  firstRec = false;
		}
...
//added for "Occupational Schools", RTC#10942, debashish.barik
  		if( contactType == "Licensed Individual" || contactType == "Business" || contactType == "Occupational School")
			{
...
catch (ex)
{
   ...
   aa.env.setValue("EMSEReturnMessage", "Error executing SQL_BATCH_DPL_RENEWAL_REMINDER" + ex.message);
   ELPLogging.debug("     EMSEReturnCode: " + ScriptReturnCodes.OTHER);
   ELPLogging.debug("     EMSEReturnMessage: " + "Error executing SQL_BATCH_DPL_RENEWAL_REMINDER. " + ex.message);
   
   var returnException = new ELPAccelaEMSEException("Error executing SQL_BATCH_DPL_RENEWAL_REMINDER " + ex.message, ScriptReturnCodes.EMSE_PROCEDURE);
   ELPLogging.fatal(returnException.toString());
   throw returnException;
}
finally 
{
	...
	if (!ELPLogging.isFatal()) {
		...
		if (ELPLogging.getErrorCount() > 0) {
			aa.env.setValue("EMSEReturnMessage", "SQL_BATCH_DPL_RENEWAL_REMINDER completed with " + ELPLogging.getErrorCount() + " errors.");
			logDebugAndEmail("EMSEReturnMessage: " + "SQL_BATCH_DPL_RENEWAL_REMINDER completed with " + ELPLogging.getErrorCount() + " errors.");
		} else {
			aa.env.setValue("EMSEReturnMessage", "SQL_BATCH_DPL_RENEWAL_REMINDER completed with no errors.");
			logDebugAndEmail("EMSEReturnMessage: " + "SQL_BATCH_DPL_RENEWAL_REMINDER completed with no errors.");					
		}
	}
...
function getRecordsArray(emseParameters){
	var sql = 
         ...
         e.expiration_status, \
         e.expiration_date, \
         Trunc(SYSDATE) - e.expiration_date AS DAYS_TO_EXPIRE, \
         Trunc(SYSDATE + 45, 'IW')          AS START_RANGE, \
         Trunc(SYSDATE +45, 'IW') + 6       AS END_RANGE \
  FROM   b1permit b \
  ...
  WHERE  b.serv_prov_code = 'DPL' \
         ...
         AND ( b.b1_appl_status = 'Current' \
                OR ( b.b1_appl_status = 'Inactive' \
                     AND b.b1_per_type = 'Real Estate' \
                     AND ( b.b1_per_sub_type = 'Salesperson' \
                            OR b.b1_per_sub_type = 'Broker' ) ) ) \
         AND e.expiration_status = 'About to Expire' \
         AND e.expiration_date >= Trunc(SYSDATE + 45, 'IW') \
         -- NOT PASSED EXPIRATION DATE \
         AND E.expiration_date <= Trunc(SYSDATE + 45, 'IW') + 6 \
         AND EXISTS (SELECT 1 \
                     FROM   accela.xapp2ref LINK, \
                            accela.b1permit p2 \

/* ------------------------------------------------------------------------------------------------------ /
| Program : SQL_BATCH_DPL_CEU_PROCESS Trigger : Batch
|
| - Stolen from the license renewal job. Modified to only process changes in CEU status
...
| - LicenseType and LicenseSubType - By License Type / LicenseSubType
| - LicenseType and LicenseSubType and Expiration Date - By License Type / LicenseSubType / ExpirationDate
|
...
| 	 OR
|   ExpirationDateFrom and ExpirationDateTo - Date range
|
/ ------------------------------------------------------------------------------------------------------ */
...
//Record Limit
var capLimit = getParam("capLimit");
if (capLimit == null || capLimit == "" || capLimit == "undefined")
	capLimit = 99999;

var licenseTypeParam = getParam("LicenseType");//"Electricians"//
var licenseSubtypeParam = getParam("LicenseSubType");//"Journeyman Electrician"//
var daysToExpired = getParam("ExpirationDateLookahead");
var expirationDate = getParam("ExpirationDate");
var fromDateParam = getParam("FromDate");//"01/01/1919"//
var toDateParam = getParam("ToDate");//"07/31/2016"//
...
try {
	...
	if (dbConfiguration) {
		...
		//for (var queryResult = dataSet.next(); queryResult != null; queryResult = dataSet.next()) {
			...
			//continue;
			try {
				if (capCount > capLimit) { // Only continue if time hasn't expired
					logMessage("WARNING", "capLimit has been exceeded. Please re-run.  " + capCount + " records processed, " + capLimit + " allowed.");
					logDebugAndEmail("WARNING", "capLimit has been exceeded. Please re-run.  " + capCount + " records processed, " + capLimit + " allowed.");
					timeExpired = true;
					break;
				}
				...
				if (b1ExpResult.getSuccess()) {
					...
					if (lpHasCondition(altId, "Right to Renew Stayed by DOR")) {
						...
						if (!createResult.getSuccess()) {
							createResult = aa.cap.createRenewalRecord(capId);
							logDebugAndEmail("ERROR: Could not create renewal record for record: " + altId + " " + createResult.getErrorMessage());
							continue;
						}
						...
						logDebug("military status : " + military);
							...
							if (renewalCap.isCompleteCap()) {
								...
								if (appTypeArray[1] == 'Plumbers and Gas Fitters') {
									...
									if (!matches(contEducWaiver, "Temporary", "Permanent")) {
										//ELPLogging.debug("No waiver for: " + altId);
										childArray = getChildren("License/DPL/Continuing Education/CE Submission", capId);
										//ELPLogging.debug("Got Child records for: " + altId);
										var licCEReq = lookup("CE_REQUIREMENTS", refLic.getLicenseType());
										if (!licCEReq) {
											//ELPLogging.debug("Unable to lookup license type for: " + altId);
										...
										for (uch in childArray) {
											...
											var ceCapId = aa.cap.getCapID(ceId.getID1(), ceId.getID2(), ceId.getID3()).getOutput();
											//ELPLogging.debug("Processing CESUB: " + ceId.getID1() + "-" + ceId.getID2() + "-" + ceId.getID3());
											var workflowResult = aa.workflow.getTasks(ceId);
											//ELPLogging.debug("Processing workflowResult ");
											if (workflowResult.getSuccess()) {
												...
												for (i in wfObj) {
													...
													if (taskName == "Intake" && taskStatus == "Credits Accepted" && getAppSpecific("Processed", ceCapId) == null) {
														//ELPLogging.debug("Getting contEducListObj ");
														var contEducListObj = aa.continuingEducation.getContEducationList(capIDScriptModel);
														//ELPLogging.debug("Got contEducListObj. Getting contEducList");
														//aa.print(contEducListObj.getOutput().length);
														contEducList = contEducListObj.getOutput();
														//ELPLogging.debug("Got contEducList:" + contEducListObj.getOutput().length);
														for (lCount in contEducList) {
															//aa.print("Hours: " + contEducList[xxx].getHoursCompleted());
															//ELPLogging.debug("Adding to totalCourseHours: " + totalCourseHours);
															totalCourseHours = totalCourseHours + contEducList[lCount].getHoursCompleted();
														}
														//ELPLogging.debug("Got totalCourseHours: " + totalCourseHours);
													}
													//ELPLogging.debug("Done with wfObj: " + i + " taskName: " + taskName + "taskStatus: " + taskStatus);
												}
												//ELPLogging.debug("Done with workflowResult");
											}
											//ELPLogging.debug("Done with ceId:" + ceId.getID1() + "-" + ceId.getID2() + "-" + ceId.getID3());
										}
				...
				aa.env.setValue("EMSEReturnMessage", "Error executing SQL_BATCH_CEU_RENEWAL" + ex.message);
				ELPLogging.debug("     EMSEReturnCode: " + ScriptReturnCodes.OTHER);
				ELPLogging.debug("     EMSEReturnMessage: " + "Error executing SQL_BATCH_CEU_RENEWAL. " + ex.message);
				...
				ELPLogging.debug("Licenses with Board not defined: " + boardNotDefined);
				ELPLogging.debug("Licenses missing LPs that were not renewed: " + licenseMissingLPs);
				ELPLogging.debug("Licenses missing expiration details that were not renewed: " + missingExpDetails);
				ELPLogging.debug("Licenses with DOR Notice conditions:" + dorCondCapCount);
				ELPLogging.debug("Plumbing and Gas Fitters Apprentices that were not renewed: " + plgsCapCount); //script id 1246
				ELPLogging.debug("Plumbing and Gas Fitter Business that were not renewed: " + plBusinessMasterInactive);
				ELPLogging.debug("Active Duty Licenses Processed: " + autoRenewCapCount);
				ELPLogging.debug("Non-Military Licenses Processed: " + renewalCapCount);
				ELPLogging.debug("Licenses not added to set because TMP renewal exists: " + renewalRecordExists);
				ELPLogging.debug("Licenses missing CE hours Processed: " + ceHoursNotMet);
				ELPLogging.debug("Licenses missing Inspector CE hours Processed: " + plInspectorHoursNotMet);

				var returnException = new ELPAccelaEMSEException("Error executing SQL_BATCH_CEU_RENEWAL " + ex.message + " " + (ELPLogging.toString()), ScriptReturnCodes.EMSE_PROCEDURE);
				ELPLogging.fatal(returnException.toString());
				throw returnException;
	...
	*/
	if (!ELPLogging.isFatal()) {
		...
		if (ELPLogging.getErrorCount() > 0) {
			aa.env.setValue("EMSEReturnMessage", "SQL_BATCH_CEU_RENEWAL completed with " + ELPLogging.getErrorCount() + " errors.");
			logDebugAndEmail("EMSEReturnMessage: " + "SQL_BATCH_CEU_RENEWAL completed with " + ELPLogging.getErrorCount() + " errors.");
		} else {
			aa.env.setValue("EMSEReturnMessage", "SQL_BATCH_CEU_RENEWAL completed with no errors.");
			logDebugAndEmail("EMSEReturnMessage: " + "SQL_BATCH_CEU_RENEWAL completed with no errors.");
		}
	}
...
function getRecordsArray(emseParameters){
	var sql = 
		"select b.SERV_PROV_CODE,b.B1_PER_ID1, b.B1_PER_ID2, b.B1_PER_ID3, b.B1_PER_GROUP, b.b1_per_type, b.b1_per_sub_type, b.B1_PER_CATEGORY, b.b1_alt_id \
			, b.B1_APPL_STATUS, e.EXPIRATION_STATUS, e.EXPIRATION_DATE\
			from B1PERMIT b\
			...
			and b.B1_PER_TYPE in ('Electricians', 'Plumbers and Gas Fitters') \
			and b.B1_PER_SUB_TYPE not in ('Course')\

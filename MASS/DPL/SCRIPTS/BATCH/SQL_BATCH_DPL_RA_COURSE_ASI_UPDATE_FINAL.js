/* ------------------------------------------------------------------------------------------------------ /
 | Program : SQL_BATCH_DPL_RA_COURSE_ASI_UPDATE Trigger : Batch
 |
...
{		
		...
		
		{
		 ...
		 editAppSpecific("APPROVED COURSE INFORMATION.Course Title", licACICourseTitle, capId);
		 editAppSpecific("APPROVED COURSE INFORMATION.Total Course Hours", licACICourseHrs, capId);
		 editAppSpecific("APPROVED COURSE INFORMATION.Course Type", licACICourseType, capId);
		 editAppSpecific("APPROVED COURSE INFORMATION.Online Course", licACIOnlineCourse, capId);
		 editAppSpecific("APPROVED COURSE INFORMATION.Board Approval Date", licACIBoardApproval, capId);
		 editAppSpecific("APPROVED COURSE INFORMATION.Meets AQB Criteria", licACIMeetsAQB, capId);
...
logDebugAndEmail("________________________________________________________________________________");
  logDebugAndEmail("Total Licenses having conflicting ASI's and have been updated: " + capCount);
  ...
  aa.env.setValue("EMSEReturnMessage", "Error executing SQL_BATCH_DPL_RA_COURSE_ASI_UPDATE" + ex.message);
 aa.print("     EMSEReturnCode: " + ScriptReturnCodes.OTHER);
 aa.print("     EMSEReturnMessage: " + "Error executing SQL_BATCH_DPL_RA_COURSE_ASI_UPDATE. " + ex.message);

  var returnException = new ELPAccelaEMSEException("Error executing SQL_BATCH_DPL_RA_COURSE_ASI_UPDATE " + ex.message + " " + (ELPLogging.toString()), ScriptReturnCodes.EMSE_PROCEDURE);
  ELPLogging.fatal(returnException.toString());
  throw returnException;
} finally {
  // close objects
  ...
  if (!ELPLogging.isFatal()) {
   ...
   aa.print("EMSEReturnCode: " + ScriptReturnCodes.SUCCESS);
    ...
    if (ELPLogging.getErrorCount() > 0) {
      aa.env.setValue("EMSEReturnMessage", "SQL_BATCH_DPL_RA_COURSE_ASI_UPDATE completed with " + ELPLogging.getErrorCount() + " errors.");
      logDebugAndEmail("EMSEReturnMessage: " + "SQL_BATCH_DPL_RA_COURSE_ASI_UPDATE completed with " + ELPLogging.getErrorCount() + " errors.");
    } else {
      aa.env.setValue("EMSEReturnMessage", "SQL_BATCH_DPL_RA_COURSE_ASI_UPDATE completed with no errors.");
      logDebugAndEmail("EMSEReturnMessage: " + "SQL_BATCH_DPL_RA_COURSE_ASI_UPDATE completed with no errors.");
    }
  }

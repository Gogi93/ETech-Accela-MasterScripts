/***********************************************************************************************************************************
* @Title 		: 	SQL_BATCH_CLOSE_ADJ_REC_DISTYPE_NO_ENDDATE
* @Author		:	Sagar Cheke
* @Date			:	04/19/2016
* @Description 	:	IF ABCC Appeal task is not active AND Disposition Type is in the following:
					- Approved
					- Disapproved
					- Warning 
					- Cancellation
					- Modification 
					- Decision Remanded to LLA   
					- 25E Case Dismissed 
					- 25E Case Memorandum            
					- Other
					THEN
					1. Close the Adjudication Record 90 days from the date the Decision task was set to "Decision Issued"
					2. Close the Adjudication Record
***********************************************************************************************************************************/
try 
{
...
var emailAddress = lookup("BATCH_STATUS_EMAIL", "DESPTYPE NO END DATE"); // This email will be set by standard choice
	...
	//{
		...
		*/
		...
		for (x in dataSet)
		{
			...
			// ABCC Appeal task is not active
			var isDispExists = getASIDetailsForAdj(capId);
			
			//Close the Adjudication Record 90 days from the date the Decision task was set to 'Decision Issued'
			var isNotActive = isABCCAppealNotActive(capId);
			if (isNotActive && isDispExists)
			{
				...
				if((daysBetweenDates >= 90) && isNotActive && isDispExists)
				{
		...
		if(asiDetailsModel)
		{
			for(ASI in asiDetailsModel)
			{
				if(asiDetailsModel[ASI].getFieldLabel() == "Disposition Type")
				{
					...
					if(exists(checklistComment, dispositionTypeArray))
					{
						isDispTypeExists = true;
					}
				}				
			}
		}	

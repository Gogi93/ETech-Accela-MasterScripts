/***********************************************************************************************************************************
 * @Title 		: 	UPDATE_OFF_QUOTA_STDCHOICE_3051
 * @Author		:	debashish.barik
 * @Date			:	05/22/2017
 * @Description 	:	UPDATE_OFF_STDCHOICE_3051
 ***********************************************************************************************************************************/
try {
...
try {
	...
	if (dbConfiguration) 
	{
	  	...
	  	for (var i in dataSet) 
		{
			...
			try {
				...
				editLookup("LLA Off Premise Licenses Issued", vMuniCOde, vCount);
	...
	aa.env.setValue("EMSEReturnMessage", "Error executing UPDATE_OFF_QUOTA_STDCHOICE_3051" + ex.message);
	ELPLogging.debug("EMSEReturnCode: " + ScriptReturnCodes.OTHER);
	ELPLogging.debug("EMSEReturnMessage: " + "Error executing UPDATE_OFF_QUOTA_STDCHOICE_3051. " + ex.message);
	...
	if (!ELPLogging.isFatal()) {
		...
		if (ELPLogging.getErrorCount() > 0) {
			aa.env.setValue("EMSEReturnMessage", "UPDATE_OFF_QUOTA_STDCHOICE_3051 completed with " + ELPLogging.getErrorCount() + " errors.");
			ELPLogging.debug("EMSEReturnMessage: " + "UPDATE_OFF_QUOTA_STDCHOICE_3051 completed with " + ELPLogging.getErrorCount() + " errors.");
			emailText = emailText + "EMSEReturnMessage: " + "UPDATE_OFF_QUOTA_STDCHOICE_3051 completed with " + ELPLogging.getErrorCount() + " errors." + br;
		} else {
			aa.env.setValue("EMSEReturnMessage", "UPDATE_OFF_QUOTA_STDCHOICE_3051 completed with no errors.");
			ELPLogging.debug("EMSEReturnMessage: " + "UPDATE_OFF_QUOTA_STDCHOICE_3051 completed with no errors.");
			emailText = emailText + "EMSEReturnMessage: " + "UPDATE_OFF_QUOTA_STDCHOICE_3051 completed with no errors." + br;
		}
	}
...
on  upper(A_All.City) = upper(r.VALUE_DESC)  and A_All.\"CATEGORY_Type\" = 'OFF_CATEGORY' \
left join JIRA_3051_Annual_Wine A_Wine \
on  upper(A_Wine.City) = upper(r.VALUE_DESC)  and A_Wine.\"CATEGORY_Type\" = 'OFF_CATEGORY' \
left join JIRA_3051_Seasonal_All S_All on  upper(S_All.City) = upper(r.VALUE_DESC)  and S_All.\"CATEGORY_Type\" = 'OFF_CATEGORY' \
left join JIRA_3051_Seasonal_Wine S_Wine on  upper(S_Wine.City) = upper(r.VALUE_DESC)  and S_Wine.\"CATEGORY_Type\" = 'OFF_CATEGORY' \

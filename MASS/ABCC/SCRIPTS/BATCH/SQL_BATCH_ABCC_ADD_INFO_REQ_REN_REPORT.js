/* ------------------------------------------------------------------------------------------------------ /
 |Created this Batch As a Part of CR 205, According to the ABCC Deficiency Weekly Reminder Script Specification for Renewal Records.
 |
 | Program : SQL_BATCH_ABCC_ADD_INFO_REQ_REN_REPORT Trigger : Batch
 |
 | Check if Record status is "Additional Information Needed" for Renewals
 |	Find the workflow task that has a status of "Additional Information Needed"
 |	- If Due Date is greater than current date and current date a multiple of 7 (days) after the Status Date email the Deficiency Letter.
...
var staticParamObj = '{\
		...
		"supplemental":   [{\
					...
					"procedure":{\
						"name":"ELP_SP_ADDINF_REN_ACCELA_QUERY",\
						"resultSet":{"list":[\
...
try
{
	...
	*/
	...
	var emseParameters = {};
		...
		var queryForDBFields = "" ;		
		if (dataSetStg != false || dataSetStg.length > 0) 
	    for (var i in dataSetStg) 	
		{
			//aa.print("dataSetStg.length: " +dataSetStg.length);	
			//ObjKeyRename(dataSetStg[i], {"B1_ALT_ID":"customID"});
...
AND PERMIT.B1_APPL_STATUS    = MAXSTATUSHISTORY.STATUS \
WHERE PERMIT.B1_PER_CATEGORY = 'Renewal' \
AND PERMIT.SERV_PROV_CODE    = 'ABCC' \
AND PERMIT.B1_APPL_CLASS     = 'COMPLETE' \
AND PERMIT.B1_APPL_STATUS    = 'Additional Information Needed'";

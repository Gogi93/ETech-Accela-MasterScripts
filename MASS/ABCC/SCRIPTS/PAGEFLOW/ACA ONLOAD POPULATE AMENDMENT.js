...
if (parentCapId)
{
  ...
  copyLicenseProfessionalXX(parentCapId, capId);
  var asiArr = new Array();
  loadAppSpecific(asiArr,parentCapId);
  copyAppSpecific4ACAAmend(asiArr);
  
  //Clear ASI
	editAppSpecific4ACA("Signature", "" ,capId);
	editAppSpecific4ACA("Title", "",capId);

	
  var addressModel = aa.address.createRefAddressScriptModel();
  var capAddressResult = aa.address.getAddressByCapId(parentCapId);
  if (capAddressResult.getSuccess()) {
    ...
    {
        aa.address.copyAddressModel(parentCap.getAddressModel(), cap.getAddressModel());                              
...
function updateContactASI(pModel,conPeople) {
...
//showMessage = true;
  ...
  for (var vCounter1 = 0; vCounter1 < contactList.size(); vCounter1++)
  {
    var vContSeq = contactList.get(vCounter1).getPeople().getContactSeqNumber();
		...
		if (contactList.get(vCounter1).getContactType() == "LLA" )
		{
			contactList.remove(vCounter1);
			continue;
		}
		
		if (contactList.get(vCounter1).getContactType() == "Manager" )
		{
			var ManagerAmend = AInfo1["Change of License Manager"];
            if(ManagerAmend == "CHECKED")
	       { 
                logDebug("ManagerAmend: " +ManagerAmend);
			   contactList.remove(vCounter1);
			   continue;
		   }
		}
    ...
    proxyBus.setInstructionAndWaterMark2TemplateFields('ABCC',DbTemplate);
        ...
        if (vCType == "Applicant"){
          ...
          contactList.get(vCounter1).setComponentName("Applicant");
        }else if (vCType == "Licensed Individual"){
          contactList.get(vCounter1).setComponentName("Contact3");
        }else if (vCType == "Authorized Representative"){
          contactList.get(vCounter1).setComponentName("Contact3");
        }else if (vCType == "Manager"){
          contactList.get(vCounter1).setComponentName("Contact2");  
        }else if (vCType == "Attorney"){
          contactList.get(vCounter1).setComponentName("Contact3");
        }else {
  ...
  var n_asit = aa.appSpecificTableScript.getAppSpecificTableGroupModel(capId).getOutput();
  var asiTables = n_asit.getTablesArray().toArray();
  for(x in asiTables)
  {
    var tableInformation = new Array(); 
    tableName = String(asiTables[x].getTableName());
    if(tableName == "DEFICIENCIES" || tableName == "ASSOCIATED LICENSES")
    {
    } 
    else
    {
...
function scrubTemplate(iDbTemplate)
{
        iDbTemplate.setEntityPKModel(null);
        return iDbTemplate;
}
...
//
{
    ...
    // get addressModel from the page flow

    ao = cap.getAddressModel();

    if (ao) {

        capAddResult = aa.address.getCapIdByAddress(ao);

        if (capAddResult.getSuccess())
        {logDebug("We have addresses");  var capIdArray = capAddResult.getOutput(); }
        else
        { logDebug("**ERROR: getting similar addresses: " + capAddResult.getErrorMessage()); return false; }
...
function copyAppSpecific4ACAAmend(thisArr) {
                ...
                while (iteFields.hasNext())
                    {
                     ...
                     if (thisArr[field.getCheckboxDesc()] != null)
                        field.setChecklistComment(thisArr[field.getCheckboxDesc()]);
                     logDebug("Updated: " + field.getCheckboxDesc() + " = " + thisArr[field.getCheckboxDesc()]);
                    }
                }
             }
    }
    ...
    function addASITable4ACAPageFlowXX_OLD(destinationTableGroupModel, tableName, tableValueArray) // optional capId
    {//Please check INCLUDES_CUSTOM method addASITable4ACAPageFlowXX
        //  tableName is the name of the ASI table
        //  tableValueArray is an array of associative array values.  All elements MUST be either a string or asiTableVal object
        ...
        for (thisrow in tableValueArray) {
            ...
            while (coli.hasNext()) {
                ...
                var xfound = false;
                for (var ii in tableValueArray[thisrow]) {
                    if (ii.toString() == colname.getColumnName().toString())
                        xfound = true;
                }
                if (xfound)  // we are passed an asiTablVal Obj
                {
                    var vValue = tableValueArray[thisrow][colname.getColumnName()].fieldValue;
                    if (vValue == null) vValue = "";
                    var args = new Array(vValue, colname);
                    var fldToAdd = aa.proxyInvoker.newInstance("com.accela.aa.aamain.appspectable.AppSpecificTableField", args).getOutput();
                    if (fldToAdd != null && fldToAdd != undefined) {
                ...
                {
                    var vValue = tableValueArray[thisrow][colname.getColumnName()];
                    if (vValue == null) vValue = "";
                    var args = new Array(vValue, colname);
                    var fldToAdd = aa.proxyInvoker.newInstance("com.accela.aa.aamain.appspectable.AppSpecificTableField", args).getOutput();
                    if (fldToAdd != null && fldToAdd != undefined) {
...
function addASITable4ACAPageFlowNew(destinationTableGroupModel, tableName, tableValueArray) // optional capId
{
    //  tableName is the name of the ASI table
    //  tableValueArray is an array of associative array values.  All elements MUST be either a string or asiTableVal object
    // 
    ...
    for (thisrow in tableValueArray) {
        ...
        while (coli.hasNext()) {
            ...
            if (typeof (tableValueArray[thisrow][colname.getColumnName()]) == "object")  // we are passed an asiTablVal Obj
            {
					...
					/*JIRA#2478 End*/
                var fldToAdd = aa.proxyInvoker.newInstance("com.accela.aa.aamain.appspectable.AppSpecificTableField", args).getOutput();
				//Modified for defect#13708. Ankush Kshirsagar
if(fldToAdd != null && fldToAdd != undefined){
                fldToAdd.setRowIndex(i);
                fldToAdd.setFieldLabel(colname.getColumnName());
                fldToAdd.setFieldGroup(tableName.replace(/ /g, "\+"));
...
function copyLicenseProfessionalXX(srcCapId, targetCapId)
{
  var lpList = aa.util.newArrayList();
  var srcList =  aa.licenseProfessional.getLicensedProfessionalsByCapID(srcCapId).getOutput();
  for (var lp in srcList){
    var lpModel = srcList[lp].getLicenseProfessionalModel();
    var vCapId = srcList[lp].getLicenseNbr();
    
    if(vCapId != srcCapId.getCustomID())
    {
      lpModel.setCapID(targetCapId);
      lpList.add(lpModel);
    }
...
function removeManagerFromAmendment(ipLicCapID) {
	if (!ipLicCapID)
		return;
	...
	for (var fvCounter in fvContacts) {
		...
		if (fvResult.getSuccess())
			logDebug("Successfully removed 'Manager' Contact.");
		else
			logDebug("Failed to remove 'Manager' Contact.");
	}
}

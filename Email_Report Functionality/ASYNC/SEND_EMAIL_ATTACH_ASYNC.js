try {
	...
	logDebug("1) Here in SEND_EMAIL_ATTACH_ASYNC: " + aa.env.getValue("eventType"));
	...
	for (w in emailArray) {
		...
		if (vEmailResult.getSuccess()) { 
			logDebug("SEND_EMAIL_ATTACH_ASYNC: " + capId.getCustomID() + ": Sending " + emailTemplate + " from " + mailFrom + " to " + conEmail);
		} else {
